...
Vault persistence for DOCX and PPTX belongs to `DocumentSaveCoordinator`. The vendored DOCX editor remains behind `DocxEditorAdapter`, with a future test fake; we reject a forever host-adapter pattern and a permanent hybrid architecture because both preserve competing state and mutation owners.
