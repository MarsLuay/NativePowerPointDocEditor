...
function findExactTextElement(text: string): HTMLElement | null {
	const pages =
		window.document.querySelector('[data-native-powerpoint-doc-editor-pages]')
		?? window.document.querySelector('.paged-editor__pages');
...
function FontRoundTripApp() {
	...
	useEffect(() => {
		...
		const run = async () => {
			...
			while (!editorRef.current?.getEditorRef()?.getView() && Date.now() < deadline) {
				if (cancelled) {
					return;
				}
		...
		void run().catch((error) => {
			...
			window.document.body.dataset.error = encodeURIComponent(message);
...
createRoot(window.document.getElementById('root')!).render(<FontRoundTripApp />);
