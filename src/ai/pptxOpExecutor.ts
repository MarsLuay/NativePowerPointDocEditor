...
async function handleSetImageCrop(context: PptxOpExecutionContext, payload: Record<string, unknown>): Promise<PptxOpExecutionResult> {
	const result = createExecutionResult();
	const slideIndex = requireNumber(payload.slideIndex, 'slideIndex');
	const shapeIndex = requireNumber(payload.shapeIndex, 'shapeIndex');
...
async function handleFitImageToFrame(context: PptxOpExecutionContext, payload: Record<string, unknown>): Promise<PptxOpExecutionResult> {
	const result = createExecutionResult();
	const slideIndex = requireNumber(payload.slideIndex, 'slideIndex');
	const shapeIndex = requireNumber(payload.shapeIndex, 'shapeIndex');
	assertSlideInRange(context.engine, slideIndex);
...
async function handleResetImage(context: PptxOpExecutionContext, payload: Record<string, unknown>): Promise<PptxOpExecutionResult> {
	const result = createExecutionResult();
	const slideIndex = requireNumber(payload.slideIndex, 'slideIndex');
	const shapeIndex = requireNumber(payload.shapeIndex, 'shapeIndex');
	assertSlideInRange(context.engine, slideIndex);
...
async function handleReplaceImage(context: PptxOpExecutionContext, payload: Record<string, unknown>): Promise<PptxOpExecutionResult> {
	const result = createExecutionResult();
	const slideIndex = requireNumber(payload.slideIndex, 'slideIndex');
	const shapeIndex = requireNumber(payload.shapeIndex, 'shapeIndex');
	const vaultImagePath = requireString(payload.vaultImagePath, 'vaultImagePath');
...
async function handleReplaceImageFromShape(context: PptxOpExecutionContext, payload: Record<string, unknown>): Promise<PptxOpExecutionResult> {
	const result = createExecutionResult();
	const slideIndex = requireNumber(payload.slideIndex, 'slideIndex');
	const shapeIndex = requireNumber(payload.shapeIndex, 'shapeIndex');
...
async function executeMediaOp(
	...
	switch (opId) {
		case 'pptx.setImageCrop':
			return await handleSetImageCrop(context, payload);
		case 'pptx.fitImageToFrame':
			return await handleFitImageToFrame(context, payload);
		case 'pptx.resetImage':
			return await handleResetImage(context, payload);
		case 'pptx.replaceImage':
			return await handleReplaceImage(context, payload);
		case 'pptx.replaceImageFromShape':
			return await handleReplaceImageFromShape(context, payload);
		default:
			return null;
	}
}
