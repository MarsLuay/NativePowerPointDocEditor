...
// Locale files live in the shared @npde/docx-editor-i18n package — both
...
// ---------------------------------------------------------------------------
// Per-locale subpath sources — `import pl from '@npde/docx-editor-i18n/pl'`
...
function renderLocaleSource(code) {
  ...
  return `/**
 * @npde/docx-editor-i18n/${code}
 *
 * ${name} (\`${code}\`) — direct locale subpath for per-locale code-splitting.
 *
 * \`\`\`ts
 * // Static — bundler ships only this locale's strings
 * import ${id} from '@npde/docx-editor-i18n/${code}';
 *
 * // Dynamic — splits into its own chunk, loaded on demand
 * const ${id} = (await import('@npde/docx-editor-i18n/${code}')).default;
 * \`\`\`
 *
...
function cmdNew(lang) {
  ...
  // per-locale subpath (`@npde/docx-editor-i18n/<lang>`) works.
