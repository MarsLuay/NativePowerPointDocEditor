#!/usr/bin/env node
// Guards the per-locale i18n bundle promise: each `@npde/docx-editor-i18n/<code>`
...
//
// Run after `bun run --filter '@npde/docx-editor-i18n' build`.
...
for (const code of codes) {
  for (const ext of ['mjs', 'js']) {
    ...
    if (!existsSync(path)) {
      errors.push(
        `✗ ${file} missing in dist/ — did \`bun run --filter '@npde/docx-editor-i18n' build\` complete?`
      );
      continue;
    }
