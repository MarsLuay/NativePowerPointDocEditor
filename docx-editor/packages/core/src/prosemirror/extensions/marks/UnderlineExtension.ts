...
import { setMark, toggleMarkWithParagraphDefaults } from './markUtils';
...
export const UnderlineExtension = createMarkExtension({
  ...
  onSchemaReady(ctx: ExtensionContext): ExtensionRuntime {
    return {
      commands: {
        toggleUnderline: () => toggleMarkWithParagraphDefaults(ctx.schema.marks.underline),
        setUnderlineStyle: (style: string, color?: TextColorAttrs) =>
          setMark(ctx.schema.marks.underline, { style, color }),
      },
      keyboardShortcuts: {
        'Mod-u': toggleMarkWithParagraphDefaults(ctx.schema.marks.underline),
      },
    };
  },
});
