...
export class ClickPositionResolver {
  ...
  private fragmentsByPage: Map<number, FragmentInfo[]> = new Map();

  /** Runs indexed by fragment blockId */
  private runsByFragment: Map<string, RunInfo[]> = new Map();
   ...
   */
  private indexFragmentsInPage(pageEl: HTMLElement, pageIndex: number): void {
   ...
   */
  private indexRunsInFragment(fragmentEl: HTMLElement, blockId: string): void {
    const runs: RunInfo[] = [];
