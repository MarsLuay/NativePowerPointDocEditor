...
export class PointerEventHandler {
  ...
  private positionResolver: ClickPositionResolver;
  private container: HTMLElement | null = null;
  private onInput?: InputEventCallback;
   ...
   */
  private getPositionFromCoords(clientX: number, clientY: number): number | null {
  ...
  private onPointerMove(e: PointerEvent): void {
    if (!this.dragging || this.dragAnchor === null) return;
   ...
   */
  private selectParagraph(pos: number): void {
   ...
   * Emit an input event.
   */
  private emitEvent(event: Parameters<InputEventCallback>[0]): void {
    this.onInput?.(event);
  }
