...
function getDomPurify(): ReturnType<typeof createDOMPurify> {
  return (domPurify ??= createDOMPurify(window as never));
}
...
function writeToClipboardFallback(content: ClipboardContent): boolean {
  ...
  const sanitized = getDomPurify().sanitize(content.html);
  const parsed = new DOMParser().parseFromString(sanitized, 'text/html');
  tempDiv.replaceChildren(...Array.from(parsed.body.childNodes));
