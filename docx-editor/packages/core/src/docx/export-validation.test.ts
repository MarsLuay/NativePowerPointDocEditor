...
describe('DOCX export validation', () => {
  ...
  test('fixed-layout tables carry explicit export widths', () => {
    const doc = createEmptyDocument();
    const columns = 3;
    const columnWidths = distributeColumnWidths(DEFAULT_TABLE_WIDTH_DXA, columns);
      ...
      content: [
        {
          ...
          content: text
            ? [{ type: 'run' as const, content: [{ type: 'text' as const, text }] }]
            : [],
        },
      ],
    });
    const row = (cells: string[]) => ({
      ...
      cells: cells.map((text, i) => cell(text, columnWidths[i])),
    });
    doc.package.document.content = [
      {
        type: 'table',
        ...
        columnWidths,
        rows: [row(['A', 'B', 'C']), row(['D', 'E', 'F'])],
      },
    ];

    const table = doc.package.document.content.find((b) => b.type === 'table') as Table;
