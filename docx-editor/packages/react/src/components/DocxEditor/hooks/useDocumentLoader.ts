...
import { seedCommentAllocator } from '@npde/docx-editor-core/prosemirror/commentIdAllocator';

/**
 * Document lifecycle: load buffer / pre-parsed doc, keep `documentRef` in
...
export function useDocumentLoader({
  ...
  // Keep the held Document ref in sync with the latest history state.
  useEffect(() => {
    documentRef.current = history.state;
  }, [history.state, documentRef]);
