...
} from '@npde/docx-editor-core/docx';
import {
  getChangedParagraphIds,
  hasStructuralChanges,
  hasUntrackedChanges,
  clearTrackedChanges,
} from '@npde/docx-editor-core/prosemirror/extensions';
import { readDocxFileFromInput, type DocxInput } from '@npde/docx-editor-core/utils';
...
export function useFileIO({
  ...
  const handleSave = useCallback(
    async (options?: { selective?: boolean }): Promise<ArrayBuffer | null> => {
      ...
      try {
        ...
        // The held Document may still have the original parsed content and won't
        ...
        let selective: SelectiveSaveOptions | undefined;
  ...
  const handleDirectPrint = useCallback(() => {
    ...
    // Build the print document with DOM APIs (avoid document.write injection risk).
    printWindow.document.open();
    printWindow.document.close();
    printWindow.document.title = 'Print';
    const pageStyleEl = printWindow.document.createElement('style');
...
`;
    printWindow.document.head.appendChild(pageStyleEl);
