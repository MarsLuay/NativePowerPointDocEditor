...
import type { Style, StyleType, Theme } from '@npde/docx-editor-core/types/document';
import {
  getStylePreviewProps,
  resolveParagraphStyleOptions,
} from '@npde/docx-editor-core/utils/stylePreview';
      ...
      <SelectContent className="min-w-[260px] max-h-[400px]">
        {styleOptions.map((style) => (
          <SelectItem
            key={style.styleId}
            value={style.styleId}
            className="overflow-hidden py-2.5 px-3"
          >
            <span className="block max-w-full truncate leading-tight" style={getStylePreviewProps(style)}>
              {getStyleName(style)}
            </span>
          </SelectItem>
        ))}
