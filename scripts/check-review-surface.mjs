...
const ROOT_SCAN_IGNORED_DIRECTORIES = new Set([
	...
	'test-results',
	'vendor',
	// A developer may keep this ignored source checkout locally, but it cannot
	// affect the Git-tracked vendor-only release surface.
	'docx-editor',
]);
...
async function checkRuntimeFiles(projectRoot, violations) {
	...
	const reactDependenciesInstalled = existsSync(path.join(projectRoot, 'node_modules', 'react'))
		&& existsSync(path.join(projectRoot, 'node_modules', 'react-dom'));
	for (const [alias, target] of Object.entries(aliases)) {
		if (
			!existsSync(target)
			&& (reactDependenciesInstalled || !REACT_ALIAS_KEYS.includes(alias))
		) {
