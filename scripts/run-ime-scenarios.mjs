...
.docxidian-fixed-probe{left:0;position:fixed;top:0;visibility:hidden}
...
</style></head><body><div class="workspace-shell"><div class="docxidian-host"><div class="editor-transform"><div class="layout-page-content"><span class="docx-run-editable" contenteditable="true">Japanese IME caret probe line.</span></div></div></div></div>
<script>(()=>{const host=document.querySelector('.docxidian-host');const probe=document.createElement('div');probe.className='docxidian-fixed-probe';host.appendChild(probe);const fixedRect=probe.getBoundingClientRect();probe.remove();const editable=document.querySelector('.docx-run-editable');editable.focus();const range=document.createRange();range.selectNodeContents(editable);range.collapse(false);const sel=window.getSelection();sel.removeAllRanges();sel.addRange(range);const ancestors=[];for(let n=editable.parentElement;n&&n!==document.documentElement;n=n.parentElement){const t=getComputedStyle(n).transform;if(t&&t!=='none')ancestors.push({tag:n.tagName.toLowerCase(),className:n.className||'',transform:t});}
