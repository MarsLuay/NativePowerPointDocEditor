...
native-powerpoint-doc-editor ${packageJson.version}
...
async function stampStylesCssVersion() {
	const stylesPath = path.join(projectRoot, "styles.css");
	const raw = await readFile(stylesPath, "utf8");
	const stamp = `/* native-powerpoint-doc-editor ${packageJson.version} */\n`;
	const next = raw.startsWith("/* native-powerpoint-doc-editor ")
		? stamp + raw.split("\n").slice(1).join("\n")
		: stamp + raw;
	if (next !== raw) await writeFile(stylesPath, next);
}
